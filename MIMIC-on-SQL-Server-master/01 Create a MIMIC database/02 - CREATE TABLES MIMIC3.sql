...
CREATE TABLE CHARTEVENTS (	-- rows=263201375
   ROW_ID NVARCHAR(MAX)  NOT NULL,
   SUBJECT_ID NVARCHAR(MAX)  NOT NULL,
   HADM_ID NVARCHAR(MAX) ,
   ICUSTAY_ID NVARCHAR(MAX) ,
   ITEMID NVARCHAR(MAX)  NOT NULL,
   CHARTTIME DATETIME NOT NULL,
   STORETIME DATETIME,
   CGID NVARCHAR(MAX) ,
   [VALUE] NVARCHAR(MAX),	-- max=91
   VALUENUM FLOAT,
   VALUEUOM VARCHAR(MAX),	-- max=17
   WARNING BIGINT ,
   ERROR BIGINT ,
   RESULTSTATUS NVARCHAR(20),	-- max=6
