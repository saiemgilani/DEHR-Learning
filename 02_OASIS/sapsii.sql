...
DROP VIEW IF EXISTS SAPSII;
CREATE VIEW SAPSII as
-- extract CPAP from the "Oxygen Delivery Device" fields
with cpap as
(
  select ie.icustay_id
    , dateadd(hour,	-1, charttime) as starttime
    , dateadd(hour, 4, charttime) as endtime
...
select ie.subject_id, ie.hadm_id, ie.icustay_id
      ...
      , round( ( datediff(day,cast(ie.intime as date),cast(pat.dob as date)) ) / 365.242 , 2 ) as age
...
select ie.subject_id, ie.hadm_id, ie.icustay_id
, SAPSII
, 1 / (1 + exp(- (-7.7631 + 0.0737*(SAPSII) + 0.9971*(LOG(SAPSII + 1))) )) as SAPSII_PROB
, age_score
, hr_score
, sysbp_score
, temp_score
